...
def build(
    ...
    print(f"base model is: {base_model} with dirs: {dir(base_model)}")
    
    print(f"base model config is: {base_cfg} and model_name_or_path is: {base_cfg._name_or_path}")
    # print(f"base model path is: {base_model.ModelPath}")
    
    if not isinstance(base_cfg, MistralConfig):
        print(f"Using MistralConfig for {base_model}")
    ...
    out_cfg = PhiConfig(**base_cfg.to_dict())
    ...
    MISTRAL_INFO = mergekit.architecture.PHI2_INFO_AGAIN_BUT_DIFFERENT # PHI2_INFO_AGAIN_BUT_DIFFERENT | PHI2_INFO
            ...
            if ".mlp." in name_format:
                for moe_index, expert in enumerate(config.experts):
                    expert_name = tensor_name.replace(
                        ".mlp.fc1", f".moe.mlp.{moe_index}.fc1"
                    )
                    expert_name = expert_name.replace(
                        ".mlp.fc2", f".moe.mlp.{moe_index}.fc2"
                    )
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(base_cfg._name_or_path)
